...
import { getListWarehouseWithPaginationStatusPayment, getDataSearchInWarehouse, getAllNumberSatusProductInWarehouse } from "../services/ProjectService"
...
const Warehouse = (props) => {
    ...
    const [currentPage, setCurrentPage] = useState(
        localStorage.getItem("Page one warehouse") ? localStorage.getItem("Page one warehouse") : 1
    ...
    const handleShowhideModalCreateWarehouse = async () => {
    ...
    const fetchProjectUser = async () => {
        let res = await getListWarehouseWithPaginationStatusPayment(currentPage, currentLimit, user.account.phone)
        if (res && +res.EC === 0) {
            setSortDataSearch(false)
            setTotalPage(+res.DT.totalPage)
            if (res.DT.totalPage > 0 && res.DT.dataProduct.length === 0) {
                setCurrentPage(+res.DT.totalPage)
                await getListWarehouseWithPaginationStatusPayment(+res.DT.totalPage, currentLimit, user.account.phone)
                if (user?.account?.groupWithRound?.name === "Customer" || user?.account?.groupWithRound?.name === "Staff" && user.account.Position) {
                    await getALlListNotification(+user.account.shippingUnit_Id, user.account.phone, user.account.Position)
                }
                if (user?.account?.groupWithRound?.name === "Dev") {
    ...
    const handlePageClick = (event) => {
        ...
        localStorage.setItem("Page one warehouse", event.selected + 1)
    ...
    const getDataWithcurrentPage = async () => {
        ...
        let res = await getListWarehouseWithPaginationStatusPayment(+currentPageAfterRefesh, +currentLimit, user.account.phone)
        if (res && +res.EC === 0) {
            let data = res.DT.dataProduct
            if (data) {
                setListWarehouse(data)
    ...
    useEffect(() => {
        window.history.pushState('', '', `?page=${localStorage.getItem("Page one warehouse")}&limit=${currentLimit}`);
        if (user?.account?.groupWithRound?.name === "Customer" || user?.account?.groupWithRound?.name === "Staff" && user.account.Position) {
            getALlListNotification(+user.account.shippingUnit_Id, user.account.phone, user.account.Position)
        }
        if (user?.account?.groupWithRound?.name === "Dev") {
    ...
    const handleSearch = debounce(async (event) => {
        ...
        if (data) {
            ...
            if (res && +res.EC === 0) {
                ...
                setListWarehouse(resultsSearch)
                if (user?.account?.groupWithRound?.name === "Customer" || user?.account?.groupWithRound?.name === "Staff" && user.account.Position) {
                    await getALlListNotification(+user.account.shippingUnit_Id, user.account.phone, user.account.Position)
                }
                if (user?.account?.groupWithRound?.name === "Dev") {
                                        ...
                                        <div className='row'>
                                            ...
                                            <div className='name-page d-block d-lg-none col-lg-12'>
                                                <div className='row'>
                                                    ...
                                                    <div className='more d-flex align-item-center justify-content-center flex-column mt-3'>
                                                                            ...
                                                                            allWarehouseLenght.map((item, index) => {
                                                                                return (
                                                                                    <div className='row d-flex justify-content-center' key={`lenght-${index}`}>
                                                                                        <div className="item col-2 d-none d-lg-block">
                                                                                        ...
                                                                                        <div className="item col-12 d-block d-lg-none " >
                                                                                            <h3>{item.AllProduct ? item.AllProduct : "0"}</h3>
                                                                                            <div className='content mb-3' style={{ fontWeight: "700" }}>
                                                                                                {t('Warehouse.informationTittleOne')}
                                                                                        ...
                                                                                        <div className="item col-12 d-block d-lg-none mobile mt-2">
                                                                                            <h3>{item.product_statusId4 ? item.product_statusId4 : "0"}</h3>
                                                                                            <div className='content mb-3' style={{ fontWeight: "700" }}>
                                                                                                {t('Warehouse.informationTittleTwo')}
                                                    ...
                                                    <div className='row '>
                                                        <div className='col-12 my-3 '>
                                                            <div className='row'>
                                                                ...
                                                                <div className='my-2 d-flex align-item-center gap-2 col-12 col-lg-3'>
                                                        ...
                                                        {sortDataSearch === false &&
                                                            <>
                                                                <div className=' d-none d-lg-block pagination  my-1 '>
                                                                    < ReactPaginate
                                                                        nextLabel="next >"
                                                                ...
                                                                <div className=' d-block d-lg-none pagination-mobile  my-1 '>
                                                                    < ReactPaginate
                                                                        nextLabel="next >"
                                                                        onPageChange={handlePageClick}
                                                                        pageRangeDisplayed={1}
