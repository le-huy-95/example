...
import { getProjectWithPaginationWithALlStatusWarehouse, getAllStatusProductWithEmployer, getDataSearchByEmplyer } from "../services/ProjectService"
...
const ManageproductsStatustwo = (props) => {
    ...
    const [currentPage, setCurrentPage] = useState(
        localStorage.getItem("infomation Page employer five") ? localStorage.getItem("infomation Page employer five") : 1
    ...
    const fetchProjectUser = async () => {
        ...
        let res = await getProjectWithPaginationWithALlStatusWarehouse(currentPage, currentLimit, +user.account.shippingUnit_Id, 2
        )
        if (res && +res.EC === 0) {
            setTotalPage(+res.DT.totalPage)
            if (res.DT.totalPage > 0 && res.DT.dataProject.length === 0) {
                setCurrentPage(+res.DT.totalPage)
                await getProjectWithPaginationWithALlStatusWarehouse(+res.DT.totalPage, currentLimit, +user.account.shippingUnit_Id, 2
                )
                if (user?.account?.groupWithRound?.name === "Customer" || user?.account?.groupWithRound?.name === "Staff" && user.account.Position) {
                    await getALlListNotification(+user.account.shippingUnit_Id, user.account.phone, user.account.Position)
                }
    ...
    const handlePageClick = (event) => {
        ...
        localStorage.setItem("infomation Page employer five", +event.selected + 1)
    }
    ...
    const fetchProjectUserAfterRefesh = async () => {
        let currentPagelocalStorage = +localStorage.getItem("infomation Page employer five")
        let res = await getProjectWithPaginationWithALlStatusWarehouse(+currentPagelocalStorage, currentLimit, +user.account.shippingUnit_Id, 2
        )
        if (res && +res.EC === 0) {
            setTotalPage(+res.DT.totalPage)
            if (res.DT.totalPage > 0 && res.DT.dataProject.length === 0) {
    ...
    useEffect(() => {
        window.history.pushState('', '', `?page=${localStorage.getItem("infomation Page employer five")}&limit=${currentLimit}`);
        if (user?.account?.groupWithRound?.name === "Customer" || user?.account?.groupWithRound?.name === "Staff" && user.account.Position) {
            getALlListNotification(+user.account.shippingUnit_Id, user.account.phone, user.account.Position)
        }
        if (user?.account?.groupWithRound?.name === "Dev") {
                                    ...
                                    <div className='table-wrapper-employer-one'>
                                        <div className='container'>
                                            <div className='title-employer-one my-3'>
                                                {t('Manage-employer.Body.Tittle-twentyEight')} ({listProjectbyUnitLenght})</div>
                                            <hr />
...
export default ManageproductsStatustwo;