...
const ManageproductsPickOk = (props) => {
    ...
    const [currentPage, setCurrentPage] = useState(
        localStorage.getItem("infomation Page employer three") ? localStorage.getItem("infomation Page employer three") : 1
    ...
    const fetchProjectUser = async () => {
        ...
        let res = await getProjectWithPaginationWithALlStatusPickup(currentPage, currentLimit, +user.account.shippingUnit_Id, 2
        )
        if (res && +res.EC === 0) {
            setTotalPage(+res.DT.totalPage)
            if (res.DT.totalPage > 0 && res.DT.dataProject.length === 0) {
                setCurrentPage(+res.DT.totalPage)
                await getProjectWithPaginationWithALlStatusPickup(+res.DT.totalPage, currentLimit, +user.account.shippingUnit_Id, 2
                )
            }
            if (res.DT.totalPage > 0 && res.DT.dataProject.length > 0) {
    ...
    const handlePageClick = (event) => {
        ...
        localStorage.setItem("infomation Page employer three", +event.selected + 1)
    ...
    const fetchProjectUserAfterRefesh = async () => {
        let currentPagelocalStorage = +localStorage.getItem("infomation Page employer three")

        let res = await getProjectWithPaginationWithALlStatusPickup(+currentPagelocalStorage, currentLimit, +user.account.shippingUnit_Id, 2
        )
        if (res && +res.EC === 0) {
            setTotalPage(+res.DT.totalPage)
            if (res.DT.totalPage > 0 && res.DT.dataProject.length === 0) {
    ...
    useEffect(() => {
        window.history.pushState('', '', `?page=${localStorage.getItem("infomation Page employer three")}&limit=${currentLimit}`);
                                    ...
                                    <div className='table-wrapper-employer-one'>
                                        <div className='container'>
                                            <div className='title-employer-one my-3'>
                                                {t('Manage-employer.Body.Tittle-twentyTwo')} ({lenghtArr})
                                            </div>
                                            <hr />
                                                            ...
                                                            <tbody key={`item-${index}`}>
                                                                <tr class="table-info">
                                                                    ...
                                                                    <td>
                                                                        ...
                                                                        {item.User_PickUp && item.Number_PickUp &&
                                                                            <span>
                                                                                {t('Manage-employer.Body.Tittle-fifteen')}                                                                                <br />
                                                                                <b>{item.User_PickUp}-{item.Number_PickUp}</b>
                                                                            </span>
                                                    ...
                                                    <tr class="table-info">
                                                        <td colSpan={14}>
                                                            <div className='d-flex align-item-center justify-content-center'>
                                                                ...
                                                                <h5>
                                                                    {t('Manage-employer.Body.Tittle-eighteen')}                                                                </h5>
...
export default ManageproductsPickOk;